...
from src.coordinates import logpolar_grid, polar_grid
from src.networks import BasicCNN, EquivariantPosePredictor, TransformerCNN
from src.transformers import Rotation, TransformerSequence
...
def resnet20(planes, cfg_data, num_caps, caps_size, depth, mode=None):
    return ETCAPS(BasicBlock, [3, 3, 3], planes=planes, cfg_data=cfg_data, num_caps=num_caps, caps_size=caps_size, depth=depth, mode=None)

def resnet32(planes, cfg_data, num_caps, caps_size, depth, mode):
    return ETCAPS(BasicBlock, [5, 5, 5], planes, num_caps, caps_size, depth, cfg_data, mode)

def resnet44(planes, cfg_data, num_caps, caps_size, depth, mode):
    return ETCAPS(BasicBlock, [7, 7, 7], planes, num_caps, caps_size, depth, cfg_data, mode)

def resnet56(planes, cfg_data, num_caps, caps_size, depth, mode):
    return ETCAPS(BasicBlock, [9, 9, 9], planes, num_caps, caps_size, depth, cfg_data, mode)

def resnet110(planes, cfg_data, num_caps, caps_size, depth, mode):
    return ETCAPS(BasicBlock, [18, 18, 18], planes, num_caps, caps_size, depth, cfg_data, mode)
